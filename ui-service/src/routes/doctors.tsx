...
function DoctorsRoute() {
  ...
  const toggleAvailability = async (d: Doctor) => {
    try {
      ...
      toast.success(
        `${d.name} marked as ${d.status === "AVAILABLE" ? "busy" : "available"}`,
      );
    } catch {
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {isLoading
                ? "Loading…"
                : `${doctors.length} doctors on the platform`}
            </p>
          </div>
        </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <button
                        ...
                        onClick={() =>
                          navigate({ to: "/doctors/$id", params: { id: d.id } })
                        }
                      >
                        {d.name}
                      </button>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between rounded-lg border p-3">
                ...
                <Switch
                  ...
                  onCheckedChange={(c) =>
                    setForm({ ...form, status: c ? "AVAILABLE" : "BUSY" })
                  }
                />
              </div>
            </div>
