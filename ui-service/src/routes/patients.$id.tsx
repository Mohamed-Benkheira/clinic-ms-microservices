...
import { MOCK_PATIENTS, MOCK_APPOINTMENTS, MOCK_NOTIFICATIONS, MOCK_DOCTORS } from "@/lib/mock-data";
...
function PatientProfileRoute() {
  ...
  if (!p) return <AppLayout><Card className="p-8 text-center">Patient not found.</Card></AppLayout>;
  ...
  return (
    <AppLayout>
      <Button variant="ghost" size="sm" className="mb-4 gap-1.5" onClick={() => navigate({ to: "/patients" })}><ArrowLeft className="h-4 w-4" /> Back to patients</Button>
        ...
        <div className="lg:col-span-4">
          <Card className="sticky top-4 p-6 text-center">
            ...
            <div className="mt-1 flex items-center justify-center gap-2">
              ...
              <Badge style={{ backgroundColor: "var(--clinic-green-soft)", color: "var(--clinic-green)" }}>ACTIVE</Badge>
            </div>
            <div className="mt-4 space-y-2 text-left text-sm text-muted-foreground">
              ...
              <div className="text-xs">Age {age} · {p.gender === "F" ? "Female" : "Male"} · DOB {p.dob || "—"}</div>
            </div>
            <div className="mt-5 flex justify-center">
              <RingProgress value={78} color="var(--clinic-teal)" label={<><span className="text-lg font-semibold">78%</span><span className="text-[10px] text-muted-foreground">adherence</span></>} />
            </div>
          </Card>
        </div>
        <div className="lg:col-span-8">
          <Tabs defaultValue="overview">
                  ...
                  {upcoming.length === 0 ? <li className="text-sm text-muted-foreground">No upcoming visits.</li> :
                    upcoming.map((a) => (
                      <li key={a.id} className="flex items-center justify-between rounded-md border p-2 text-sm">
                        <div>{dayjs(a.datetime).format("MMM D, HH:mm")} · {MOCK_DOCTORS.find((d) => d.id === a.doctorId)?.name}</div>
                        <Button size="sm" variant="ghost" className="text-[var(--clinic-red)]">Cancel</Button>
                      </li>
                    ))}
                </ul>
              </Card>
            ...
            <TabsContent value="appts">
              <Card className="p-4">
                <Table>
                  <TableHeader><TableRow><TableHead>Date</TableHead><TableHead>Doctor</TableHead><TableHead>Reason</TableHead><TableHead>Status</TableHead></TableRow></TableHeader>
                  <TableBody>
                    {appts.map((a) => (
                      <TableRow key={a.id}>
                        <TableCell className="text-sm">{dayjs(a.datetime).format("MMM D, YYYY HH:mm")}</TableCell>
                        <TableCell className="text-sm">{MOCK_DOCTORS.find((d) => d.id === a.doctorId)?.name}</TableCell>
            ...
            <TabsContent value="notifs">
              <Card className="p-5">
                <ul className="space-y-3">
                  {notifs.length === 0 ? <li className="text-sm text-muted-foreground">No notifications.</li> : notifs.map((n) => (
                    <li key={n.id} className="flex items-start gap-3">
                      <span className="mt-1.5 h-2 w-2 flex-shrink-0 rounded-full" style={{ backgroundColor: n.status === "SENT" ? "var(--clinic-teal)" : "var(--clinic-red)" }} />
                      <div className="flex-1"><div className="text-sm">{n.message}</div><div className="text-xs text-muted-foreground">{dayjs(n.timestamp).format("MMM D, HH:mm")} · {n.channel}</div></div>
                    </li>
                  ))}
                </ul>
              </Card>
            ...
            <TabsContent value="docs">
              <Card className="py-12 text-center">
                ...
                <Button className="mt-3" variant="outline">Upload document</Button>
              </Card>
            </TabsContent>
          </Tabs>
        </div>
