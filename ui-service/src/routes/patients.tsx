...
function PatientsRoute() {
  ...
  const [form, setForm] = useState<Patient>({ id: "", name: "", email: "", phone: "", gender: "M", dob: "", notes: "" });
  ...
  const submit = () => {
    ...
    if (editing) {
      setPatients((ps) => ps.map((x) => (x.id === editing.id ? { ...form, id: editing.id } : x)));
      toast.success("Patient updated");
    } else {
      setPatients((ps) => [{ ...form, id: `p${Date.now()}` }, ...ps]);
  ...
  const doDelete = () => {
    if (deleteId) {
      setPatients((ps) => ps.filter((p) => p.id !== deleteId));
      ...
      <div className="space-y-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{patients.length} patients in your clinic</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            {canEdit && <Button size="sm" onClick={openAdd} className="gap-1.5"><Plus className="h-4 w-4" /> Add patient</Button>}
          </div>
        </div>
          ...
          <div className="mt-4 overflow-x-auto">
            {loading ? (
              ...
              <div className="py-12 text-center">
                ...
                {canEdit && <Button variant="outline" size="sm" className="mt-3" onClick={openAdd}>Add your first patient</Button>}
              </div>
            ) : (
              <TooltipProvider>
                <Table>
                      ...
                      <TableRow key={p.id}>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-3">
                            ...
                            <button className="text-sm font-medium hover:underline" onClick={() => navigate({ to: "/patients/$id", params: { id: p.id } })}>{p.name}</button>
                          </div>
                        </TableCell>
                        <TableCell className="text-sm text-muted-foreground">{p.email}</TableCell>
                        <TableCell>
                          <Badge variant="outline" style={{ backgroundColor: p.gender === "F" ? "var(--clinic-pink-soft)" : "var(--clinic-blue-soft)", color: p.gender === "F" ? "var(--clinic-pink)" : "var(--clinic-blue-strong)" }}>{p.gender}</Badge>
                        </TableCell>
                        <TableCell className="text-sm">{p.phone}</TableCell>
                        <TableCell className="max-w-[200px]">
                          <Tooltip>
                            <TooltipTrigger asChild><div className="truncate text-sm text-muted-foreground">{p.notes || "—"}</div></TooltipTrigger>
                            <TooltipContent className="max-w-xs">{p.notes || "No notes"}</TooltipContent>
                          </Tooltip>
                        </TableCell>
                        <TableCell className="text-right">
                          {canEdit ? (
                            <div className="flex justify-end gap-1">
                              <Button variant="ghost" size="icon" className="h-8 w-8 text-[var(--clinic-blue)]" onClick={() => openEdit(p)}><Edit className="h-4 w-4" /></Button>
                              <Button variant="ghost" size="icon" className="h-8 w-8 text-[var(--clinic-red)]" onClick={() => setDeleteId(p.id)}><Trash2 className="h-4 w-4" /></Button>
                            </div>
                          ) : (
                            <Button variant="outline" size="sm" className="gap-1" onClick={() => navigate({ to: "/patients/$id", params: { id: p.id } })}><FileText className="h-3 w-3" /> View</Button>
                          )}
                        </TableCell>
                      </TableRow>
                    ))}
        ...
        <Dialog open={modalOpen} onOpenChange={setModalOpen}>
          <DialogContent>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1.5"><Label>Medical notes</Label><Textarea rows={3} value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} /></div>
            </div>
            <DialogFooter>
        ...
        <Dialog open={!!deleteId} onOpenChange={(o) => !o && setDeleteId(null)}>
          <DialogContent>
            ...
            <DialogFooter>
              ...
              <Button className="bg-[var(--clinic-red)] hover:bg-[var(--clinic-red)]/90" onClick={doDelete}>Delete</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
