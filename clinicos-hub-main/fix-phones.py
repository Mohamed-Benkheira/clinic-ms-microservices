...
def main():
    ...
    for i, patient in enumerate(patients):
        ...
        current_phone = patient.get("phone", "")
        
        # Skip if already has + format
        if current_phone.startswith("+") and current_phone != "+213796081530" and current_phone != "+213770090216":
            continue
            
        phone = phones[i % 2]
        try:
            result = update_patient(token, patient_id, phone)
            print(f"Updated patient {patient_id} ({patient.get('full_name')}): {current_phone} -> {phone}")
            updated += 1
