...
function DoctorProfileRoute() {
  ...
  if (!d)
    return (
      <AppLayout>
        <Card className="p-8 text-center">Doctor not found.</Card>
      </AppLayout>
    );
  ...
  const myPatients = Array.from(new Set(appts.map((a) => a.patientId)))
    .map((pid) => MOCK_PATIENTS.find((p) => p.id === pid))
    .filter(Boolean);
        ...
        <div className="lg:col-span-4">
          <Card className="sticky top-4 p-6 text-center">
            ...
            <Badge
              ...
              style={{
                backgroundColor:
                  d.status === "AVAILABLE" ? "var(--clinic-green-soft)" : "var(--clinic-red-soft)",
            ...
            <div className="mt-4 space-y-2 text-left text-sm text-muted-foreground">
              <div className="flex items-center gap-2">
                <ShieldCheck className="h-4 w-4" /> {d.license}
              </div>
              <div className="flex items-center gap-2">
            ...
            <div className="mt-5 grid grid-cols-3 gap-2 border-t pt-4 text-center">
              ...
              <div>
                <div className="text-lg font-semibold">{myPatients.length}</div>
                    ...
                    {g.items.length === 0 ? (
                      ...
                      g.items.map((a) => {
                        ...
                        return (
                          ...
                          >
                            <div>
                              <span className="font-mono font-semibold text-[var(--clinic-blue-strong)]">
                ...
                <ul className="divide-y">
                  {myPatients.map((p, i) => (
                    <li
                      ...
                      style={{ padding: 12, borderRadius: 4, cursor: "pointer" }}
                ...
                <Card className="flex items-center justify-around p-5">
                  ...
                  <div className="space-y-2 text-sm">
                    <div>
                      ...
                      <div className="text-lg font-semibold text-[var(--clinic-red)]">8%</div>
                    </div>
                    <div>
                      <div className="text-xs text-muted-foreground">Avg duration</div>
                      <div className="text-lg font-semibold">28m</div>
                    </div>
                    <div>
                      <div className="text-xs text-muted-foreground">Satisfaction</div>
                      <div className="text-lg font-semibold text-[var(--clinic-green)]">4.8/5</div>
                    </div>
                  </div>
                </Card>
              </div>
