...
import { useDoctors, useUpdateDoctor, useUpdateDoctorAvailability } from "@/hooks/use-doctors";
...
const SPECIALTIES = ["General Practice", "Cardiology", "Pediatrics", "Neurology", "Orthopedics", "Dermatology"];
const EMPTY_FORM: Omit<Doctor, "id"> = { name: "", specialty: "General Practice", license: "", phone: "", email: "", status: "AVAILABLE" };
...
function DoctorsRoute() {
  ...
  const openEdit = (d: Doctor) => {
    setEditing(d);
    setForm({ name: d.name, specialty: d.specialty, license: d.license, phone: d.phone, email: d.email, status: d.status });
    setOpen(true);
  };
    ...
    <AppLayout>
      <div className="space-y-5">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {doctors.map((d, i) => (
              <motion.div key={d.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.08 }} whileHover={{ y: -4, scale: 1.02 }}>
                <Card className="p-5">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <button className="text-base font-semibold hover:underline" onClick={() => navigate({ to: "/doctors/$id", params: { id: d.id } })}>
                        {d.name}
                      </button>
                      <p className="text-sm text-muted-foreground">{d.specialty}</p>
                      <Badge className="mt-1" style={{
                        backgroundColor: d.status === "AVAILABLE" ? "var(--clinic-green-soft)" : "var(--clinic-red-soft)",
                        color:           d.status === "AVAILABLE" ? "var(--clinic-green)"      : "var(--clinic-red)",
                      }}>
                        {d.status}
                      </Badge>
                    </div>
                  ...
                  <div className="mt-4 space-y-1.5 text-sm text-muted-foreground">
                    ...
                    <div className="flex items-center gap-2"><ShieldCheck className="h-3.5 w-3.5" /> {d.license}</div>
                  </div>
                  <div className="mt-4 flex items-center justify-between">
                    ...
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-[var(--clinic-blue)]" onClick={() => openEdit(d)}>
          ...
          <DialogContent>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between rounded-lg border p-3">
                ...
                <Switch checked={form.status === "AVAILABLE"} onCheckedChange={(c) => setForm({ ...form, status: c ? "AVAILABLE" : "BUSY" })} />
              </div>
            </div>
            <DialogFooter>
              <Button variant="ghost" onClick={() => setOpen(false)} disabled={updateMutation.isPending}>Cancel</Button>
        ...
        <Dialog open={!!delId} onOpenChange={(o) => !o && setDelId(null)}>
          <DialogContent>
            ...
            <p className="text-sm text-muted-foreground">This will remove the doctor from the platform.</p>
            <DialogFooter>
              <Button variant="ghost" onClick={() => setDelId(null)}>Cancel</Button>
              <Button className="bg-[var(--clinic-red)] hover:bg-[var(--clinic-red)]/90" onClick={() => setDelId(null)}>
                Confirm removal
              </Button>
            </DialogFooter>
          </DialogContent>
